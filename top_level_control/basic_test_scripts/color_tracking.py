...
- Fusion: 70% HSV + 30% LAB (configurable)
"""
...
class EnhancedBallTracker:
    ...
    Fusion weights: 70% HSV, 30% LAB (adjustable via hsv_weight)
    """
    ...
    def __init__(self):
        ...
        # Fusion weight: 0.6 = 60% HSV, 40% LAB
        self.hsv_weight = 0.6
    ...
    def build_fused_mask(self, frame, return_debug=False):
        """
        Build a cleaned HSV+LAB fused mask for the given frame.
        ...
        Returns:
            mask or (mask, debug) if return_debug is True
        """
        frame_normalized = self.preprocess(frame)
        mask_hsv = self.detect_hsv(frame_normalized)
        mask_lab = self.detect_lab(frame_normalized)
    ...
    def set_fusion_weight(self, hsv_weight):
        ...
        Args:
            hsv_weight: 0.0 to 1.0 (0.7 = 70% HSV, 30% LAB)
        """
