"""
Test Stereo Sync Verification
==============================
Verifies hardware synchronization by measuring frame-pair time deltas.
Does NOT enable trigger mode — use setup_trigger_and_verify.py for that.

ASSUMES: External trigger mode is already enabled (via setup_trigger_and_verify.py
         or the Arducam AMCap app).

MEASUREMENT:
  - Measures the time delta between left and right frame arrivals.
    If synced: delta is tiny and consistent (< 2ms, just USB transfer gap)
    If NOT synced: delta drifts and varies widely

  - After collecting frames, displays:
    1. Live side-by-side view with timing overlay
    2. Final graph: frame-pair time deltas + trigger interval
...
def main():
    ...
    # --- Step 1: Configure resolution + codec ---
    print(f"\n--- Step 1: Configure MJPG {CAMERA_WIDTH}x{CAMERA_HEIGHT}@100fps ---")
    ...
    if match:
        print(f"  OK: Both cameras matched at {s_left['width']}x{s_left['height']} {s_left['fourcc']}")
    else:
        print(f"  WARNING: Camera settings do NOT match!")
    ...
    # Data collection
    deltas_ms = []         # Time between left.read() and right.read() completing
    trigger_intervals = [] # Time between consecutive frame pairs
    frame_count = 0
    last_pair_time = None
    ...
    try:
        ...
        fig.suptitle('Stereo Camera Sync Verification', fontsize=14, fontweight='bold')
