...
def open_camera(device_id, width, height):
    """Open camera with CAP_DSHOW and force MJPG codec."""
    ...
    print(f"  Opened camera {device_id}: {actual_w}x{actual_h} @ {cap.get(cv.CAP_PROP_FPS):.0f}fps")
    return cap
...
def save_frames_single_camera(camera_name):
    ...
    cap = open_camera(camera_device_id, width, height)
...
def save_camera_intrinsics(camera_matrix, distortion_coefs, camera_name):
    ...
    out_filename = os.path.join(CAMERA_PROPERTIES_DIR, camera_name + '_intrinsics.dat')
...
def save_frames_two_cams(camera0_name, camera1_name):
    ...
    cap0 = open_camera(calibration_settings[camera0_name], width, height)
    cap1 = open_camera(calibration_settings[camera1_name], width, height)
    ...
    while True:
        ...
        # grab/retrieve for sync — both frames from same trigger pulse
        cap0.grab()
        cap1.grab()
